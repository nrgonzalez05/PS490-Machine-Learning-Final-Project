...
#     Top-N covariates for primary outcome (racial resentment)
#     across all four treatments. Main text figure.
#
#   fig_11_varimp_outcome_grid.png
#     Top-N covariates for each outcome x treatment combination
#     (main analysis outcomes only; welfare in appendix).
#     Appendix figure — useful for showing why welfare fails.
#
#   fig_12_varimp_welfare_flag.png
#     Side-by-side: racial resentment vs welfare spending
#     for national retrospective treatment only.
#     Highlights anomalously high party ID / ideology importance
#     for welfare — the empirical grounding for the absorption
#     explanation in appendix A2.
#
# STRATEGY:
#   - Fit on df1 (first MICE imputation) — standard for
#     diagnostics; importance is structural, not estimate-level.
#   - Same forest spec as cf_cont_fn in mpsa_3_cf_main.R:
#     num.trees = 2000, min.node.size = 5, honesty = TRUE
#   - Variable names mapped to human-readable labels via
#     a covariate label lookup table defined below.
#   - Top 15 variables shown per panel by default (N_TOP).
#
...
N_TOP   <- 15    # top N variables to display per panel
SEED    <- 123
N_TREES <- 2000

# Outcomes for the main grid (welfare appendix-only)
outcomes_main <- c(
  "O1_rr_index",
  "O5_aidpoor_binary",
...
# ============================================================
# COVARIATE HUMAN-READABLE LABEL MAP
# Maps raw ANES 2024 variable names to interpretable labels.
# All variable codes verified against ANES 2024 Time Series
# variable list (anes_timeseries_2024_varlist_20250808).
# ============================================================
...
covariate_labels <- c(
  ...
  # ── CF extra controls — political engagement ─────────────
  ...
  # ── Presidential / institutional approval ────────────────
  ...
  # ── Emotional responses (about country) ─────────────────
...
# Helper: clean up model.matrix suffixes and map to labels
label_covariate <- function(varnames) {
  # model.matrix appends numeric suffixes for factor levels
  # Strip trailing digits/suffixes after last underscore or
  # numeric run to recover the base variable name
...
# ============================================================
# FOREST FIT + IMPORTANCE EXTRACTION FUNCTION
# Identical spec to cf_cont_fn in mpsa_3_cf_main.R
# Returns tidy tibble of importance scores with labels
# ============================================================
...
# ============================================================
# RUN: all continuous treatments x all outcomes
# Using df1 for diagnostics (standard)
# ============================================================
...
# National retrospective treatment only.
# Shows anomalously high party ID / ideology importance
# for welfare relative to racial resentment — the
# empirical basis for the absorption explanation in A2.
# ============================================================
...
cat("\n", strrep("=", 60), "\n")
cat("TOP 5 VARIABLES — PRIMARY OUTCOME (RACIAL RESENTMENT)\n")
cat(strrep("=", 60), "\n\n")
...
varimp_all %>%
  ...
  select(treatment_label, rank, variable_label, importance_norm) %>%
  mutate(importance_norm = round(importance_norm, 3)) %>%
  print(n = Inf)

cat("\n", strrep("=", 60), "\n")
cat("TOP 5 VARIABLES — WELFARE SPENDING (APPENDIX)\n")
cat(strrep("=", 60), "\n\n")

varimp_all %>%
  filter(outcome == "O4_welfare_binary",
         treatment == "C_nat_econ_last") %>%
  slice_max(importance, n = 10) %>%
  select(rank, variable_label, importance_norm) %>%
  mutate(importance_norm = round(importance_norm, 3)) %>%
  print(n = Inf)

cat("\n", strrep("=", 60), "\n")
cat("PARTY ID / IDEOLOGY RANK COMPARISON\n")
cat("National retrospective: Racial resentment vs Welfare\n")
cat(strrep("=", 60), "\n\n")
...
varimp_all %>%
  filter(
    treatment == "C_nat_econ_last",
    outcome   %in% c("O1_rr_index", "O4_welfare_binary"),
    variable_label %in% party_ideo_vars
  ) %>%
  select(outcome_label, variable_label, rank, importance_norm) %>%
  mutate(importance_norm = round(importance_norm, 3)) %>%
  arrange(outcome_label, rank) %>%
  print(n = Inf)

cat("\nDone. All variable importance outputs saved to:", out_dir, "\n")